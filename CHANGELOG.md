...
- **Relative placement: a declaration can state where it goes instead of computing it.** `resistor:R1 "1 kΩ" right-of VIN by 190 #amber` places R1 beside VIN and level with it. Six relations — `right-of`, `left-of`, `above`, `below`, `aligned-x with`, `aligned-y with` — each optionally carrying a `by` distance, each addressing a component or one of its terminals. `at (x, y)` is unchanged and unaffected; the two forms may be mixed freely in one document, and a document that uses only coordinates compiles to the bytes it always did.
- Directions measure from the referenced **body**, not its origin, so `by 190` is 190 units of clear space between facing edges — what an author asking for room means, and orientation-correct without asking about orientation. An `id.port` reference measures from that terminal instead, so `below U1.pin3` lines a part up with a pin rather than a package.
- An axis no relation constrains **inherits it from the first reference**. `right-of A` therefore means beside A _and level with_ A. Stating an alignment as well overrides the inherited axis, which is how a part takes its column from one component and its row from another.
- **This is a lowering pass, not a layout engine.** Relations resolve to exactly one `at (x, y)` before the AST is finished, so the netlist, the design rules, the source map, and the renderer never learn that relative placement exists. The suite pins that as a property rather than a claim: for a corpus of documents, the relative form and the absolute form it lowers to compile to **byte-identical SVG**. Nothing decides where a part goes except a relation the author wrote — auto-layout would forfeit the determinism and the control that make coordinates worth keeping.
- Resolution is one Kahn topological sort and one linear evaluation. No solver, no relaxation, no iteration to a fixed point. Forward references resolve, so declaration order is not a constraint; a cycle is a compile error naming its members in order, not an under-constrained system to be minimised. Evaluation order provably cannot reach the coordinates — a position is a function of its references' positions alone — so the reported `placements` array is sorted by source line, the only order an author can predict.
- `compileSchematic` returns `placements`: the coordinates each relative declaration resolved to, and the relations as written with port aliases folded to canonical terminals. Empty for a document that used only `at (x, y)`. A host offering "freeze to absolute" reads it rather than re-deriving it. Also exported from `@schemd/core/placement`.
- **`routing`: what the router had to do.** `attempts` counts rip-up retries and is zero for any document that routed on the first pass. `rippedUp` names each torn-up trace and the retry that tore it up. `congestion` reports occupancy per routing-hash cell, sorted by column then row, with `SCHEMATIC_CONGESTION_CELL_SIZE` describing the grid — a renderable artifact for a host that wants to show where a canvas is tight, read out of the hash the router already filled rather than measured a second time.
- Two new `limits` fields, both resolved through the existing budget: `placementDepth` (default 64) bounds the longest reference chain, and `routingAttempts` (default 12) bounds routing passes. `routingAttempts: 1` disables rip-up exactly, restoring pre-0.5 routing behaviour.
- `routeSchematicConnections` and `validateSchematicGeometry` return routes _and_ the report. `routeConnections` and `validateDocumentGeometry` keep their signatures and return only routes, so no existing call site changes.

### Changed

- **Routing retries around contention instead of giving up at the first trace that will not fit.** The first pass is still the greedy source-order route, unchanged, and is what every document that already compiled still takes. When a trace fails, the traces placed so far are torn up, the failed trace is promoted to the front of the next pass, and the rest are reordered shortest-span-first — a criticality proxy, since a trace whose endpoints are nearly level has one usable channel and no slack while a long one can bend around what is already there.
- A **reversal bus now routes at twelve wires, from ten.** Eleven settles in two passes and twelve in seven.
- **Thirteen is where it stops, and the reason is worth stating plainly: that is a limit of the router's channel model, not of the order traces are placed in.** Widening the fence changes nothing, and twenty thousand random declaration orders were searched at that width without finding one that routes — so no reordering heuristic, this one included, reaches it. It is pinned by a test so the claim stays honest and a future channel-model change has a target.
- The two gzip budgets rise to 34 KiB and 37 KiB, from 32 and 35. Relative placement does not tree-shake out of the compile path and was never going to — it is a language feature, so every document that parses carries the lowering pass. Recorded here rather than absorbed, and paid for by two features rather than by drift.

### Verified

- Ten mutants join the kill gate, one per load-bearing property: an unconstrained axis must inherit the first reference; a direction must measure from the body rather than the origin; the two axis defaults must stay distinct; the depth budget must be enforced; reported placements must be in source order; a declaration must wait for every reference it names; the first routing pass must be source order; a failed trace must be promoted to the front; the attempt budget must be enforced; congestion must be reported in a stable order. Twenty-four of twenty-four are killed.
- **A pre-existing mutant survived once rip-up existed, and the test it defeated was strengthened rather than the mutant retired.** `terminal approaches are reserved before any wire is placed` was killed in 0.4.0 by a reversal-bus fixture that failed to compile without the reservation. With retries in place the same fixture compiles anyway, so the defect became invisible. The fixture now asserts `routing.attempts === 0`: a bus of that size is an ordinary figure and must not need a retry. This is the general hazard the feature introduces — rip-up can paper over a router defect that used to fail loudly — and it is why any test asserting a figure compiles should also assert how hard it was.
- Determinism is asserted, not argued: a congested document compiles to one distinct output over a thousand runs, and its report is stable over fifty. The retry path has no clock, no randomness, no scored permutation, and no iteration over an unordered container; the index is rebuilt each pass so no residue of a torn-up trace survives into the next.
- The fuzz suite gains random placement graphs — forward, backward, self, and cyclic references — and asserts every one either resolves with finite coordinates and one report entry per declaration, or is rejected with a line-accurate diagnostic. Never a hang, never an unhandled throw.
...
- Clean documents are unaffected, and this is measured rather than asserted: the dense 16×16 routing benchmark is unchanged, because a document that routes on the first pass executes the code it executed before rip-up existed. The twelve-wire contended bus — which no source order can route at all — compiles in about 31 ms.
