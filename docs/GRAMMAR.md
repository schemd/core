...
All route families are collision checked. Straight and cubic paths cannot penetrate unrelated component bodies or earlier connector labels; orthogonal paths route around bodies and labels while pricing earlier unrelated wire channels as soft occupancy. Transformed endpoint-marker footprints participate in the same rule. Separate nets may cross only as a strict perpendicular orthogonal crossing, which receives a bridge on the later trace. Collinear overlap, endpoint contact, diagonal or cubic crossing, and bridge clusters too dense to render are errors. Same-net contacts do not receive bridges and may reuse their own channels.
